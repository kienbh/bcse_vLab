...
# bcse-id (id.bcse-vju.com) federated SSO — secrets generated by
...
def _inject_bcse_id_secrets(c: paramiko.SSHClient) -> None:
    """Write BCSE_ID_CLIENT_SECRET / BCSE_ID_JWT_SECRET / BCSE_ID_WEBHOOK_SECRET
    into SV14's .env.prod from app_secrets.json. Idempotent — appends missing
    keys, replaces existing ones in place. Runs as `student` (the .env.prod
    owner) so no sudo needed."""
    if not APP_SECRETS_FILE.exists():
        raise FileNotFoundError(
            f"app_secrets.json missing at {APP_SECRETS_FILE} — run "
            "'Server Management/register_sv14_hardware_lab.py' first to "
            "register the sv14-hardware-lab OIDC client on bcse-id."
        )
    blob = json.loads(APP_SECRETS_FILE.read_text())
    needed = {
        "BCSE_ID_CLIENT_SECRET": blob.get("sv14-hardware-lab"),
        "BCSE_ID_JWT_SECRET": blob.get("BCSE_ID_JWT_SECRET"),
        "BCSE_ID_WEBHOOK_SECRET": blob.get("webhook_sv14-hardware-lab"),
    }
    missing = [k for k, v in needed.items() if not v]
    if missing:
        raise RuntimeError(
            f"app_secrets.json missing keys: {missing}. "
            "Re-run register_sv14_hardware_lab.py."
        )

    log("  injecting BCSE_ID_* into .env.prod (from app_secrets.json)")
    py = (
        "import json, re, pathlib\n"
        "p = pathlib.Path('.env.prod')\n"
        "s = p.read_text()\n"
        f"vals = {json.dumps(needed)}\n"
        ...
        "        s += line + '\\n'\n"
        "p.write_text(s)\n"
        "print('BCSE_ID_* injected:', ','.join(vals))\n"
    )
    run(
        c,
        f"cd {SV14_REMOTE}/infrastructure/sv14 && python3 -c {shlex.quote(py)}",
        check=True,
    )
...
""")
        ...
        # ADR-0014: inject bcse-id client/webhook/JWT secrets from app_secrets.json
        # into .env.prod so the FastAPI backend can verify id_tokens signed by
        # the federated IdP on SV08. Without this, SSO callbacks fail with
        # signature-verify errors.
        _inject_bcse_id_secrets(c)
